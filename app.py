...
<style>
    ...
    .stForm input:focus, .stForm textarea:focus {
        border-color: #4299e1;
...
def authenticate_user():
    ...
    # Define credentials
    CREDENTIALS = {
        "Vimarsh": {"password": "Vimarsh121", "role": "Customer"},
...
def main():
    ...
    st.markdown("""
    <div class="main-header">
        <h1>🛡️ Insurance Claim Pro</h1>
        <p>AI-Powered Claim Management & Processing</p>
    </div>
    """, unsafe_allow_html=True)
...
def process_claim_with_agents(claim: Claim):
    ...
    if st.session_state.agent_system:
        with st.spinner("🤖 AutoGen agents analyzing your claim..."):
            try:
                # Use Streamlit's built-in async support
                agent_results = st.session_state.agent_system.process_claim_with_agents(claim)
